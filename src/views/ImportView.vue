...
export default {
  ...
  data() {
    return {
      ...
      isOverwrite : true,
      errors : [],
      theModelType : 'Model package (.cairis)',
      modelTypes : ['Model package (.cairis)',
                    'Model file (.xml)',
                    'Project data',
                    'Requirements',
                    'Risk Analysis',
  ...
  methods : {
    onImport(evt) {
      evt.preventDefault();
      if (this.checkForm()) {
        if (this.theModelType == 'Model package (.cairis)'  || this.theModelType == 'Persona characteristics (Workbook)' || this.theModelType == 'User goals (Workbook)') {
          this.importBinary();
        }
        else {
          this.importFile();
