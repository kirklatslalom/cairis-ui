...
-->
  ...
  <div class="controlstructure">
    ...
    <b-container fluid>
      <b-row>
        ...
        <b-col v-if="theEnvironmentName != ''">
          <b-form-group label="Filter" label-for="csFilter" :label-cols="2" >
            <dimension-select ref="csFilter" id="csFilter" dimension="control" :environment="theEnvironmentName" initial="all" includeall v-on:dimension-select-change="filterSelected" v-on:dimension-items-updated="filtersLoaded" />
          </b-form-group>
        </b-col>
        <b-col v-if="theEnvironmentName != ''">
    ...
    <b-container fluid>
      <graphical-model v-if="theEnvironmentName != ''" ref="graphicalModel" :api="csURI" v-on:graphical-model-url="nodeClicked"/>
    </b-container>
  </div>
</template>
...
export default {
  computed : {
    csURI() {
      return "/api/dataflows/control_structure/environment/" + this.theEnvironmentName + "/filter_name/" + this.theFilterName ;
    },
    panelParameters() {
  ...
  methods : {
    nodeClicked(url) {
      ...
      if (['assets','dataflows','trust_boundaries'].indexOf(this.theSelectedDimension) == -1) {
        return;
      }
      axios.get(url,{
